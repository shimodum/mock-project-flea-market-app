...
- **取引中商品の確認機能**
  ...
  - メッセージの未読数の表示  
- **取引チャット機能**
  - 商品ごとの取引チャット画面でメッセージのやり取りが可能  
  - メッセージ投稿時のバリデーション（400文字以内、JPEG/PNGの画像アップロード）  
  - エラーメッセージ表示（ファイル形式の不一致、文字数オーバーなど）  
- **別取引遷移機能**
  - 取引チャット画面のサイドバーから別の取引へ遷移できる  
- **自動ソート機能**
  - チャット画面のメッセージは新規メッセージが来た順に表示される  
- **取引商品新着通知確認機能**
  - 新規購入された商品の未読メッセージに通知マークを表示  
  - 通知マークから未読メッセージが確認できる  
- **評価機能**
  - 取引完了後、出品者と購入者が相互に評価を行える  
  - 評価は1〜5の数値で行い、平均値がマイページに表示される  
  - 小数点以下は四捨五入される  
