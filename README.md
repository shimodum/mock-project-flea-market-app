...
## プロジェクト概要
本プロジェクトは、前回提出したフリマアプリに追加機能を実装した改良版です。  
Laravel を使用し、アイテムの出品、購入、チャット取引、評価機能など、  
より実用的なフリマ体験ができるアプリケーションを目指しています。  
...
---
## ダミーユーザー情報
本アプリケーションの動作確認用に、以下のダミーユーザー情報を用意しています。  
`php artisan migrate:fresh --seed` 実行後、以下のアカウントでログインできます。  

| ユーザー名 | メールアドレス         | パスワード  | 郵便番号   | 住所             | 建物名             |
|-------------|-----------------------|-------------|------------|------------------|------------------|
| **User1**   | user1@example.com    | password1   | 111-1111   | 東京都港区六本木1-1-1  | 六本木ヒルズ       |
| **User2**   | user2@example.com    | password2   | 222-2222   | 東京都渋谷区恵比寿2-2-2 | 恵比寿タワー     |
| **User3**   | user3@example.com    | password3   | 333-3333   | 東京都港区虎ノ門3-3-3   | 虎ノ門ビル     |

### 備考:
- **User1**：CO01～CO05の商品を出品しています。  
- **User2**：CO06～CO10の商品を出品しています。  
- **User3**：何も紐づけられていないユーザーです。  

> ※ 上記のユーザーは全員メール認証済みの状態です。すぐにログインし、動作を確認できます。  
> ※ MailHogによるメール認証の挙動を確認したい場合は、「会員登録」画面から任意のメールアドレスとパスワードを入力して、新規登録してください。  
---
